...
updated_census_data <- census_data %>% 
  mutate(province = 
         ...
         age=round(age)
  ) %>%  dplyr::select (province, religion_important, age)
updated_census_data <- na.omit(updated_census_data)

updated_census_data <- updated_census_data %>%
  group_by(province, religion_important) %>%
  summarise(n=n()) %>% 
  mutate(prop = n /sum(n))
...
survey_data_LARGE <- 
  ...
  mutate(vote_liberal = ifelse(p3==1,1,0),
         ...
         pref_party = case_when(
           q11 == 1 ~ "Liberal",
           q11 == 2 ~ "Conservatives",
           q11 == 3 | 4 | 5 | 6 | -9 | -8 | -7 | 7 | 8 | 9 | 10 ~ "Other"),
         lean_party = case_when(
           q11 == 1 ~ "Liberal",
           q11 == 2 ~ "Conservatives",
           q11 != 1 | 2 ~ "Other"),
        gov_satisfaction = case_when(
           q13 == 1 ~ "Satisfied",
           q13 == 2 ~ "Satisfied",
           q13 == 3 ~ "Not Satisfied",
           q13 == 4 ~ "Not Satisfied",
         ...
         more_immigrant_spending = ifelse(q27_e == 1, 1,0),
         more_defence_spending = case_when(
           q27_d == 1 ~ "Spend More",
           q27_d != 1~ "Spend Less or the same"),
         more_crime_spending = case_when(
           q27_c == 1 ~ "Spend More",
           q27_c != 1 ~ "Spend Less or the same"),
         more_environment_spending = case_when(
           q27_b == 1 ~ "Spend More",
           q27_b != 1 ~ "Spend Less or the same"),
         kept_promises_trudeau = case_when(
           q46 == 1 ~ "Agree",
           q46 == 2 ~ "Agree",
           q46 == 3 ~ "Disagree",
           q46 == 4 ~ "Diasgree"),
         ...
         voted_2019 = case_when(
           p2 == 1 ~ "Voted",
           p2 == 2  || 3 || 4 || 5 || 6 ||-9 || -8 || -7~  "Didn't Vote"
         ),
...
dplyr::select(vote_liberal, age, province, pref_party, lean_party, gov_satisfaction, know_premier, volunteer, know_financemin, voting_duty, more_refugees, more_immigrants, perceived_winner, perceived_winner_riding, fed_bettereconomy, better_economy, more_immigrant_spending,more_defence_spending, more_crime_spending, more_environment_spending, kept_promises_trudeau, voted_2015, party_2015, completed_highschool, religion_important, en_first_language, household_income, voted_2019, opinion_federal_performance, married, union)
...
LARGE_logit_model <- glm(vote_liberal~age + as.factor(province) + as.factor(lean_party)  + as.factor (gov_satisfaction) + as.factor(know_premier) + as.factor(know_financemin) + as.factor(volunteer) + as.factor(voting_duty)+ as.factor(more_refugees) + as.factor(more_immigrants) +as.factor(perceived_winner) +as.factor(perceived_winner_riding) +as.factor(fed_bettereconomy) +as.factor(better_economy) +as.factor(more_immigrant_spending) +as.factor(more_defence_spending) + as.factor(more_environment_spending) +as.factor(more_crime_spending) +as.factor(kept_promises_trudeau) +as.factor(voted_2015) +as.factor(party_2015) + as.factor(completed_highschool) + as.factor(religion_important) + as.factor(en_first_language) + household_income + as.factor(opinion_federal_performance) + as.factor(married) + as.factor(union), survey_data_LARGE, family=binomial)
summary(LARGE_logit_model)
#AIC = 913.49

##BIC model selection
sel.var.bic <- step(LARGE_logit_model, trace = 0, k = log(length(survey_data_LARGE$vote_liberal)), direction = "both") 
select_var_bic<-attr(terms(sel.var.bic), "term.labels")
select_var_bic

#results from LARGE model --> Lean party, party 2015, opinion_federal_performance
##BIC Model
BIC_stepwise_model <- glm(vote_liberal ~ as.factor(lean_party) + as.factor(party_2015) + as.factor(opinion_federal_performance), survey_data_LARGE,
                          family=binomial)
summary(BIC_stepwise_model)
BIC(BIC_stepwise_model)
#AIC is 475.13
#BIC is 507.349

##AIC model selection
sel.var.aic <- step(LARGE_logit_model, trace = 0, k = 2, direction = "both") 
select_var_aic<-attr(terms(sel.var.aic), "term.labels")
select_var_aic
#results: province, lean_party, know_financemin, perceived_winner, more_defence_spending, kept_promises_trudeau, voted_2015, party_2015, religion_importance,
#opinion_federal performance
#AIC model
AIC_stepwise_model <- glm(vote_liberal ~ as.factor(province) + as.factor(lean_party) + as.factor(know_financemin) + as.factor(perceived_winner) + as.factor(more_defence_spending) + as.factor(kept_promises_trudeau) + as.factor(voted_2015) + as.factor(party_2015) +as.factor(religion_important) + as.factor(opinion_federal_performance), survey_data_LARGE, family=binomial)
summary(AIC_stepwise_model)
#AIC is 434.63
#BIC is 540.2749

##BIC modified to include province and religion
BIC_step_mod <- glm(vote_liberal ~ as.factor(province) + as.factor(religion_important) +as.factor(lean_party) + as.factor(party_2015) + as.factor(opinion_federal_performance), survey_data_LARGE,
                          family=binomial)
summary(BIC_step_mod)

##poststratification -- unsure of how to apply to data
census_data$estimate <- AIC_stepwise_model %>% 
  predict(newdata = updated_census_data)

#AIC = 911.25 (-votemtters), 904.28(-controlofpower), 903.49(-union), 903.49 (-married), 899.97 (-household income), 1372.4
